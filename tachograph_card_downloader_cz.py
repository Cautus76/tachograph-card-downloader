...
rs = readers()
if not rs:
    raise SystemExit("❌ Nebyla nalezena žádná čtečka")
...
def read_current_file():
    ...
    while offset < 65535:
        ...
        while True:
            ...
            # Karta oznámila správnou délku
            if sw1 == 0x6C:
                requested = sw2 if sw2 else 0x80
                continue

            # Některé karty odmítnou příliš velký požadavek
        ...
        raise RuntimeError(
            "READ BINARY selhal: SW=" + sw_text(sw1, sw2)
        )
...
def append_tlv(fid, suffix, value):
    ...
    if len(value) >= 0xFFFF:
        raise RuntimeError("Soubor je pro DDD TLV příliš velký")
...
def download_ef(
    ...
    if (sw1, sw2) != (0x90, 0x00):
        if optional:
            print(
                f"  – {name}: na kartě není "
                f"(SW={sw_text(sw1, sw2)})"
            )
            return False

        raise RuntimeError(
            f"{name}: SELECT selhal, SW={sw_text(sw1, sw2)}"
        )
    ...
    try:
        if signed:
            ...
            if (sw1, sw2) != (0x90, 0x00):
                raise RuntimeError(
                    f"HASH selhal, SW={sw_text(sw1, sw2)}"
                )

        file_data = read_current_file()

        if not file_data:
            raise RuntimeError("soubor neobsahuje žádná data")
        ...
        if signed:
            ...
            if (sw1, sw2) != (0x90, 0x00):
                raise RuntimeError(
                    f"podpis selhal, SW={sw_text(sw1, sw2)}"
                )
        ...
        if optional:
            print(f"  – {name}: vynecháno ({error})")
            return False
        raise


print("🔌 Čtečka:", rs[0])
print("💳 Karta připojena")
print()

try:
    # Soubory v hlavním adresáři karty
    print("=== HLAVNÍ SOUBORY ===")
    ...
    # Generace 1
    print()
    print("=== TACHOGRAF GEN1 ===")

    sw1, sw2 = select_aid(AID_GEN1)

    if (sw1, sw2) != (0x90, 0x00):
        raise RuntimeError(
            "Gen1 aplikaci nelze vybrat: SW=" +
            sw_text(sw1, sw2)
        )
    ...
    # Generace 2
    print()
    print("=== TACHOGRAF GEN2 ===")

    sw1, sw2 = select_aid(AID_GEN2)

    if (sw1, sw2) != (0x90, 0x00):
        raise RuntimeError(
            "Gen2 aplikaci nelze vybrat: SW=" +
            sw_text(sw1, sw2)
        )
    ...
    if not ddd:
        raise RuntimeError("Nebyla stažena žádná data")

    desktop = os.path.expanduser("~/Desktop")
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"KARTA_RIDICE_{timestamp}.DDD"
    output_path = os.path.join(desktop, filename)

    counter = 1
    while os.path.exists(output_path):
        filename = f"KARTA_RIDICE_{timestamp}_{counter}.DDD"
        output_path = os.path.join(desktop, filename)
        counter += 1
    ...
    print()
    print("======================================")
    print("✅ DDD SOUBOR BYL VYTVOŘEN")
    print("📄", output_path)
    print("📦 Velikost:", len(ddd), "bajtů")
    print("🧩 Počet TLV bloků:", blocks)
    print("🔐 SHA-256:", checksum)
    print("======================================")

except Exception as error:
    print()
    print("❌ STAHOVÁNÍ BYLO ZASTAVENO")
    print(error)
    print("Soubor nebyl uložen jako hotový DDD.")
